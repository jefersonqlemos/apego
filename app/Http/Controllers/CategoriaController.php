...
class CategoriaController extends Controller
{
    ...
    public function categoriaFeminino($id)
    {
        ...
        $produtos = Produto::where('generos_idgeneros', 2)->orWhere('generos_idgeneros', 3)->where('categorias_idcategorias', $id)->where('quantidade', '>', 0)->orderBy('idprodutos', 'desc')->groupBy('variante_tamanho')->where("cidades_idcidades", "=", $idcidade)->paginate(9);
        $marcas = Marca::all();
    ...
    {
        $idcidade = Cookie::get('cookieCidade');
        $produtos = Produto::where('generos_idgeneros', 1)->orWhere('generos_idgeneros', 3)->where('categorias_idcategorias', $id)->where('quantidade', '>', 0)->orderBy('idprodutos', 'desc')->groupBy('variante_tamanho')->where("cidades_idcidades", "=", $idcidade)->paginate(9);
        $marcas = Marca::all();
        $categorias = Categoria::all();
        $tamanhos = Tamanho::all();
        return view('shop')->with(compact('produtos', 'tamanhos', 'categorias', 'marcas'));
